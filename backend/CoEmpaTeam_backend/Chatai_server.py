...
class DialogueRequest(BaseModel):
    user_input: str
    history: List[str]
    avatars: List[str]          # role names, e.g., ["Alice", "Benji", "Caden"]
    participant_role: str       # participant role name, e.g., "Alice"


# Load role data for the specified avatars from avatars.json
def load_avatar_data(names):
    with open("Avatars_prompt.json", "r", encoding="utf-8") as f:
        data = json.load(f)
    return {name: data[name] for name in names if name in data}


# Format the role card into a prompt-friendly string
...
"""
    ...
    for msg in dialogue.history:
        ...
        messages.append({
            "role": "system",
            "content": (
                f"The previous message was from the participant {dialogue.participant_role}. "
                f"Now generate a response only from {gpt_avatars[0]} and {gpt_avatars[1]}. "
                f"Do NOT speak for {dialogue.participant_role}."
            )
        })
    ...
    try:
        ...
        # Keep only GPT avatar utterances; filter out invalid speakers (e.g., the participant)
        valid_speakers = set(gpt_avatars)
        filtered_reply = [turn for turn in reply_json if turn.get("speaker") in valid_speakers]

        # If everything gets filtered out, return a safe fallback (prevents Unity errors)
